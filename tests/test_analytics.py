...
from conftest import end_match  # noqa: E402
...
def _play(store, mid, players, start, end, **kw):
    ...
    end_match(store, _snap(mid, end, players, state="WaitingPostMatch", **kw))
...
def test_longest_infantry_kill_excludes_vehicle_weapons(tmp_path):
    ...
    store.finalize_open_match(reason="test", confirmed=True)  # reads serve only finished matches
    store.close()
...
def test_switching_vehicles_closes_and_reopens(tmp_path):
    ...
    store.finalize_open_match(reason="test", confirmed=True)
    store.close()
...
def test_match_end_flushes_open_sessions(tmp_path):
    ...
    end_match(store, _vsnap("m1", "2026-07-15T10:06:00+00:00",
                            [_veh("v1", "BP_T72", 0.0, "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa")],
...
def test_vehicles_used_aggregates_in_profile(tmp_path):
    ...
    store.finalize_open_match(reason="test", confirmed=True)   # flush the open session
    store.close()
...
def test_activity_grid_buckets_by_dow_and_hour(tmp_path):
    ...
    store.finalize_open_match(reason="test", confirmed=True)  # profile reads finished matches only
    store.close()
