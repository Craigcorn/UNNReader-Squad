...
| 6 | **Seeding is excluded per layer, at the source.** All servers (2–3) mix seeding and competitive play, so agent config carries a seeding-layer pattern list (default matches layers named `*Seed*`). A seeding match is never recorded: no file, no stats, no upload. Cheat detection still runs live during seeding (plugins consume snapshots, not recordings) | Decided |
