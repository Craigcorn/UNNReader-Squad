...
| UAV (`BP_CommandActor_UAV_MQ9_C`): position is the point; a shootable actor | none beyond the common fields | `Health` and `Dead_0` are not recorded — decision D18, reversing D17: a UAV shot down on 2026-09-07 left both untouched (1000, false) through the kill and a 16 s linger; the shoot-down is the common `actionDestroyed` (§9). No command actor records a `Health` at all, so the read-by-name rule never has to exempt one family from another. The layout also carries `HealthComponent`, `Min Flight Speed`, `Max Flight Speed`, `Actual Flight Speed` and `Height`, none of which is recorded |
| Commander drone call actor (`BP_CommandActor_Drone_C`) | `ownerEosId` (string) | `SQ PC` → player state — the commander who called it (2026-09-07, T14); on the pawn the same-named field holds the deployer (§7). Its `Health` is not recorded — decision D18 extended: it read 100 throughout a call, and the drone's health is the pawn's (§7). This actor's root position reads (0, 0, z) and means nothing; it never flips `Action Destroyed` and outlives its window (still present 18 min past it, 2026-09-07) — the drone's own life is the pawn's (§7) |
...
| `BP_CommandActor_Drone_C` (archived 09-02) | BlueprintGeneratedClass | yes — content, exists only during a call | `SQ PC` |
...
through a kill), and with them the drone call actor's `Health`, which
read 100 through a whole call (D18 extended to every command actor,
2026-09-07); the commander drone's spawner deployable and drone item
...
| `BP_CommandActor_Drone_C` | archive, 09-02 | `Distance` Float; `Team` Int; `DamageInstigatorController` WeakObject; `Action` Class; `Action Destroyed` Bool; `Destroy Delay after Action Destroyed` Double; `SQ PC` Object |
