...
| T8 | Does the commander implementation match the probe on a real match? → six-player acceptance run with the probe as oracle | W19, the former session A content | 6 players; W18 (the commander implementation) landed | the command-assets probe and run sheet; the drone probe alongside for drones-doc D9 (commander drone: `SQ PC` persistence, health, linger, `Command Action` set) — the probe needs `BatteryLifetimeMax` added to its field list first (read by a one-off layout read on 2026-09-05, absent from the probe) | specified — blocked on W18 |
