...
- D17: keep `dead` on the UAV command actor beside `health` — 2026-09-05; its flip has never been observed (no UAV or aircraft was shot down in any session) and is observed under T9's shoot-down half; no last-hit field exists on any command actor, so who shot it down stays unrecorded.
...
| T8 | Does the commander implementation match the probe on a real match? → six-player acceptance run with the probe as oracle | W19, the former session A content | 6 players; W18 (the commander implementation) landed | the command-assets probe (`cmd_probe_b1.py`: box `/tmp` and the Misc archives; it graduates to `scripts/probes/` before this test is harness-ready) and run sheet, with the marker `Action` pointer added to its marker reads for D13; the drone probe alongside for drones-doc D9 (commander drone: `SQ PC` persistence, health, linger, `Command Action` set; and what `SQ PC` on the call actor `BP_CommandActor_Drone_C` holds — spec §6 names it `ownerEosId` on the pawn's evidence) | specified — blocked on W18 |
| T9 | The unobserved commander branches: shoot-down attribution, re-stamp at a commander change, claim after a step-down, command zones → journal entries when the branches occur | W20, the open commander items that gate nothing | a claimable commander — on the test box the claim needs at least four players online (Craig, 2026-09-05; the manager's rule read as three squads of two), so never solo; opportunistic | observation and the journal; no tooling | harness-ready |
...
| T13 | What do the action configs' `DisplayName` and `Description` strings hold on the loaded CDOs? → the two strings read per config at a claim | D15 (Craig 2026-09-05: record the display name once observed); the 09-02 sweep recorded the fields' existence, never their values | a claimable commander (the configs load when a claim resolves) — rides T8 | two FString reads added to the command-assets probe's CDO dump at T8 prep | specified 2026-09-05 |
