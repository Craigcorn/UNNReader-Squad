...
| T12 | What does a command marker's `Action` class pointer hold — on a request marker before and after approval, and on each footprint marker during a call? → change-only rows from the probe across a request's life and a call | D13 (Craig 2026-09-05: record it once observed); spec §5 | solo half: one squad leader placing and approving a request; call half: a commander calling assets (rides T8) | `scripts/probes/marker_action.py` | harness-ready 2026-09-05 |
| T13 | What do the action configs' `DisplayName` and `Description` strings hold on the loaded CDOs? → the two strings read per config at a claim | D15 (Craig 2026-09-05: record the display name once observed); the 09-02 sweep recorded the fields' existence, never their values | a claimable commander (the configs load when a claim resolves) — rides T8 or any session that runs T9 | two FString reads added to the command-assets probe's CDO dump before that session | specified 2026-09-05 |
