...
| W15 | Commander capture spec: a contract-only `docs/command-assets-spec.md` covering all five wire surfaces of the workstream (commander block, game-state rules, marker geometry, `commandActions`, `drones` with its 4 Hz key), plus viewer rules and doctor names; it carries no state and no open-items table — an unresolved interpretation is cited by tracker id with its meaning (rule 10), so nothing in it moves when a row closes; then a fresh-session comparison against the journal on a written checklist | drafted and reviewed 2026-09-05: a fresh-session comparison on §12 returned 23 corrections (the artillery family's field names were the journal's unspaced transcription; four struct names had no archived evidence, now reflected and archived; four recorder-side inferences; a checklist citing a section that no longer existed) — all applied, one deliberate deviation recorded in §12 (identity fields omitted, not `null`) | `docs/command-assets-spec.md` | W17, the implementation plan |
...
| D13 | Record the command marker's `Action` class pointer as `markers[].action` (found 2026-09-05 on `BP_MapMarker_CommandMaster_C`, +0x328): a direct join from a request or geometry marker to the `CommandAction_*` it belongs to, hence to the cooldown entry — one additive field on a class the spec already reads | Craig | W17 (cheapest inside the same implementation) | none; the field is reflected and archived | spec §5 and §10; journal "Reflected live 2026-09-05" |
...
| T8 | Does the commander implementation match the probe on a real match? → six-player acceptance run with the probe as oracle | W19, the former session A content | 6 players; W18 (the commander implementation) landed | the command-assets probe and run sheet; the drone probe alongside for drones-doc D9 (commander drone: `SQ PC` persistence, health, linger, `Command Action` set; and what `SQ PC` on the call actor `BP_CommandActor_Drone_C` holds — spec §6 names it `ownerEosId` on the pawn's evidence) | specified — blocked on W18 |
