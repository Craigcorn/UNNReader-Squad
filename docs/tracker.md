...
| W20 | Open commander items that gate nothing: A1 shoot-down attribution (`LastHitBy`), B2 bomb-circle radii (cosmetic), R8 command zones, C1 direct category-gate test (optional), the re-stamp rule for a still-cooling asset at a commander change and the claim rule after a step-down (unobserved branches) | open; A1's drone half answered 2026-09-05 (`LastHitBy` on the drone pawn resolved to the shooter at the kill), the aircraft half remains | `docs/command-assets.md` open table and run sheet | observed via test T9; W15's spec cites them by id where a field's interpretation depends on them |
...
| D13 | Record the command marker's `Action` class pointer as `markers[].action` (found 2026-09-05 on `BP_MapMarker_CommandMaster_C`, +0x328): a direct join from a request or geometry marker to the `CommandAction_*` it belongs to, hence to the cooldown entry — one additive field on a class the spec already reads. Craig 2026-09-05: yes, but only after test T12 has observed what the pointer holds; the spec gains the field when T12 decodes | Craig (answered; T12 gates the spec edit) | W17 (cheapest inside the same implementation) | the field exists on every command marker class (all eight, T11); the action CDOs carry the reverse joins `MapMarkerClass` and `CommandActor`, so either side can join — the marker's pointer is the one that resolves per placed marker; what it reads on a request marker before approval is T12's solo half, and that it names the calling action on a geometry marker is T12's call half, both run before the spec's text is final (2026-09-07) — the recorder would emit the class name as read either way | spec §5 and §10; journal "Reflected live 2026-09-05" |
...
- D15: record the action configs' `DisplayName` on each `commander.cooldowns.actions[]` entry, not the `Description` — 2026-09-05; the field joins the spec once test T13 has read the strings (never captured before), replacing the SquadCalc class-name mapping in spec §9.
