...
| D19 | `dead` and `lastHitBy` on the 4 Hz drone position line, each emitted only when set — the killer of a drone becomes the `lastHitBy` of the first sample carrying `dead`, definitive to a quarter second | Craig | W17 (the drones list is new in W18 and bumps the packed stream anyway) | recommended 2026-09-07 after T14: under several shooters `LastHitBy` moved 1.1 s after the death, inside the 1 Hz full-frame gap; cost one bool and one pointer chain per drone per sample (about five small reads), ~50 B of raw JSON per sample only after a hit, ~15 KB raw and about a kilobyte on disk per drone death | spec §7 |
