...
| D13 | Record the command marker's `Action` class pointer as `markers[].action` (found 2026-09-05 on `BP_MapMarker_CommandMaster_C`, +0x328): a direct join from a request or geometry marker to the `CommandAction_*` it belongs to, hence to the cooldown entry — one additive field on a class the spec already reads | Craig | W17 (cheapest inside the same implementation) | the field exists on every command marker class (all eight, T11); the action CDOs carry the reverse joins `MapMarkerClass` and `CommandActor`, so either side can join — the marker's pointer is the one that resolves per placed marker; what it reads on a request marker before approval, and that it names the calling action on a geometry marker, is observed under T8 — the recorder would emit the class name as read either way | spec §5 and §10; journal "Reflected live 2026-09-05" |
| D14 | Artillery fire-plan extras on the creep and mortar actors: `Pre Warning Delay` (double), `Current Prewarning Shells` (int, a progress counter like `Current Barrage`), `First`/`Main Barrage Height Variance` (doubles), `Barrage Interval` (a timer handle, not recordable) — beyond the seven plan fields decision 6 agreed | Craig | W17 if wanted inside the same implementation; otherwise never — low importance | none; layouts archived 08-30 and 09-02 | spec §10 (T11 enumeration) |
| D15 | The action configs' own display text: `DisplayName` and `Description` (strings on every `CommandAction_*` CDO), recordable with the four config values on `commander.cooldowns.actions[]`. Would give the viewer the game's own asset names instead of the SquadCalc class-name mapping in spec §9 | Craig | W17 — medium importance: removes an external dependency from the viewer | none; three CDO layouts archived 09-02 | spec §9 (asset display names), §10 |
| D16 | The action configs' placement bounds: `MinimumDistance`, `MaximumDistance`, `MaxAngleFromBase` (floats on the CDOs; the journal's asset table quotes them per asset), recordable with the config values so the viewer can draw where an asset could have been placed | Craig | W17 if wanted; low importance | none | spec §10; journal "Per-asset action configs" |
| D17 | Keep `dead` (`Dead_0`) on the UAV command actor beside `health`, added to the spec on 2026-09-05 from the 08-30 layout as the strike family's pair; decision 6 had said only "health if present" for the UAV | Craig | W17 | none; the field is archived and the UAV can be shot down (player-confirmed 2026-09-04) | spec §6 UAV row |
