...
export function useKillFeed() {
  ...
  useEffect(() => {
    ...
    const unsub = useViewerStore.subscribe((s) => {
      ...
      // LIVE ONLY. Replay drives the feed from the pre-computed, playhead-
      // filtered timeline (useReplayKillFeed) — running the incremental diff
      // over scrubbed frames re-emits already-seen deaths with no attacker
      // ("?") and never rewinds. The mode-flip reset above still runs so the
      // DiffState is clean when we return to live.
