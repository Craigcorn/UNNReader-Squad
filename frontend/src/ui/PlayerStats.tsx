...
// squadreader.com (the `build:apple` build) vs an operator's own box (the plain
// build). The central site has a dedicated replay browser at /replays, so the
// match list is removed from this page there — it was the 11th of 13 tabs,
// which is most of why finding a replay was hard. An operator's local UI has no
// such page, so it keeps every tab.
const IS_CENTRAL = import.meta.env.VITE_THEME === "apple";
...
const ALL_TABS: { key: string; label: string }[] = [
  ...
  // No "Vehicle K/D" board. It was withdrawn deliberately, not lost: it is
  // derived from the kill feed, where the attacker is usually identified by the
  // server log's EOS id rather than the id the rest of the platform is keyed
  // by, so the two have to be matched up by inference (see central/alias.py).
  // Vehicle kills are almost entirely log-attributed, which made that board the
  // one most dependent on the inference and the least able to stand on its own.
  // "Vehicles Destroyed" below is a different, directly-measured thing.
  { key: "score", label: "Combat" },
  { key: "objective", label: "Objective" },
  { key: "revives", label: "Revives" },
  // "Vehicles Destroyed" = enemy vehicles wrecked, straight from the game's own
  // vehicleKills counter on the scoreboard — measured, not inferred.
...
// What this page actually offers. On central the match list lives at /replays.
const TABS = IS_CENTRAL
  ? ALL_TABS.filter((t) => t.key !== "matchlist")
  : ALL_TABS;
...
const LEADER_TABS = new Set([
  "elo", "kills", "kd", "inf_kd", "score", "objective", "revives",
...
function statValue(stat: string, v: number): string {
  if (stat === "kd" || stat === "inf_kd") return v.toFixed(2);
  return int(v);
}
...
export function PlayerStats({ inline = false }: { inline?: boolean } = {}) {
  ...
  // The site's search box has always navigated here as /stats/?q=<name>, and
  // this component only ever read `tab` and `server` — so the query was dropped
  // on the floor and the visitor landed on an empty search field.
  ...
  const [q, setQ] = useState(() =>
    inline ? new URLSearchParams(window.location.search).get("q") || "" : "");
            ...
            <tr key={m.match_id} className="ps-row" onClick={() => onOpen(m.match_id)}>
              ...
              <td className="ps-watch-cell">
                {m.has_replay
                  ...
                  : <span className="ps-dim ps-arch"
                          title="Replays are kept only for the newest matches per server. Stats are kept forever.">
                      stats only</span>}
              </td>
            </tr>
          );
  ...
  return (
    <div className="ps-profile">
      <div className="ps-prof-head">
        ...
        {m.has_replay ? (
          // Same tab, deliberately: opening a new one is why the browser's Back
          // button never returned you to the list you came from.
          <a className="ps-tab ps-watch-btn"
             href={`/replay/?mode=replay&id=${encodeURIComponent(m.match_id)}`}>▶ Watch replay</a>
        ) : null}
    ...
    <table className="ps-mini ps-matches">
      <thead>
        <tr>
          ...
          <th className="ps-num">ELO</th><th></th>
        </tr>
      </thead>
      <tbody>
        {rows.map((m) => (
          <tr key={m.match_id}>
            ...
            {/* "Find the match my friend played in and watch it" is the most
                natural thing to want here, and until the profile query started
                returning has_replay it was impossible end to end: these rows
                rendered and not one of them could be opened. */}
            <td className="ps-watch-cell">
              {m.has_replay
                ? <a className="ps-watch-btn"
                     href={`/replay/?mode=replay&id=${encodeURIComponent(m.match_id)}`}
