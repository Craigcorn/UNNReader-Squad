...
# The SAME end-of-match policy the recorder uses for .sqrx rotation. Importing
# it rather than restating it is the point: this file used to answer "has the
# match ended?" differently from recorder.py, and the looser of the two answers
# is what put fabricated results on the site.
...
def _m006_match_end_confirmation(conn: sqlite3.Connection) -> None:
    """Record whether the match END was actually observed.

    Without this a mid-match snapshot is stored as the final score and a winner
    is inferred from it. Measured on the live archive before this landed: 123 of
    821 matches carried a fabricated result, and one was verified against its own
    replay — 8023 frames, every one InProgress, no terminal state at all, stored
    as "273-203, team 1 won".

    Values: finalized (observed) / unverified (never observed) / legacy (rows
    that pre-date this; trust unknown). Existing final rows are stamped legacy
    rather than unverified: most of them are fine, and a blanket downgrade would
    throw away good history to punish the bad.
    """
    have = _columns(conn, "matches")
    for col in ("end_state", "end_reason"):
        if col not in have:
            conn.execute(f"ALTER TABLE matches ADD COLUMN {col} TEXT")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_matches_end_state "
                 "ON matches(end_state)")
    conn.execute("UPDATE matches SET end_state='legacy' "
                 "WHERE status='final' AND end_state IS NULL")
...
# Tickets need their own ceiling: _COUNT_MAX was tuned for player counters and
# lets any value up to 5000 through, so a torn read lands inside the band and is
# stored as a score. Squad starts a match somewhere between 100 and 1000
# depending on the layer; 1500 leaves headroom. Mirrored in central.db._ticket.
_TICKET_MAX = 1_500
# One tick is ~0.5-2 s. Tickets bleed a few per minute and drop by at most a few
# dozen on a big vehicle loss, so a jump this large is a bad read, not a game
# event. Used to stop a torn value latching into the running row.
_TICKET_JUMP_MAX = 400


def _tickets(v: Any) -> Optional[int]:
    """A plausible ticket count, else None."""
    n = _i(v)
    if n is None or n < 0 or n > _TICKET_MAX:
        return None
    return n
...
class StatsStore:
    def __init__(self, db_path: Path, *, server_id: str,
        ...
        with self.conn:
            self.conn.execute(
                "UPDATE matches SET status='final', winner_team=NULL, "
                "end_state='unverified', "
        ...
        # Consecutive ticks carrying a recognised "not playing" state, and the
        # first such snapshot. See record_tick.
        self._inactive_ticks: int = 0
        self._end_snap: Optional[dict] = None
    ...
    def record_tick(self, snap: dict) -> None:
        ...
        # Is this tick EVIDENCE that the match ended, or merely the absence of
        # evidence that it is still running? The two used to be treated the
        # same, and that is what let a single torn read end a match and publish
        # whatever score happened to be current.
        #
        # Only a state we recognise as "not playing" counts, and only after
        # MATCH_TRANSITION_CONFIRM_TICKS of them in a row — the same policy the
        # recorder has always applied to .sqrx rotation. Anything else (an
        # unreadable matchState, an empty matchId while the state still says
        # InProgress, an unknown/future string) is ambiguous and holds the match
        ...
        if match_id is not None:
            self._inactive_ticks = 0
            self._end_snap = None
        elif is_known_inactive:
            self._inactive_ticks += 1
            # Keep the FIRST confirming frame: by the third the world may
            # already be tearing down.
            if self._end_snap is None:
                self._end_snap = snap
        else:
            # Ambiguous. Break any run in progress and do nothing else.
            self._inactive_ticks = 0
            self._end_snap = None
            return

        end_confirmed = (match_id is None
                         and self._inactive_ticks >= MATCH_TRANSITION_CONFIRM_TICKS)
        if match_id is None and not end_confirmed:
            # A confirmed-inactive tick that has not yet reached the threshold.
            # The match stays open; nothing is written.
            return
            ...
            if match_id is None:
                if open_id is not None:
                    self._finalize_match(
                        cur, open_id, self._last_snap,
                        f"confirmed matchState={match_state!r}",
                        end_state="finalized", end_snap=self._end_snap)
                    finalized.append(open_id)
                self._flush_vehicle_sessions(cur)
                new_open = None
            else:
                if open_id is not None and open_id != match_id:
                    # The old match's end was never observed — a new id simply
                    # appeared. Its score is wherever it had got to.
                    self._finalize_match(cur, open_id, self._last_snap,
                                         "superseded: matchId changed",
                                         end_state="unverified")
                    finalized.append(open_id)
    ...
    def finalize_open_match(self, *, reason: str, confirmed: bool = False) -> None:
        ...
        shutdown and end-of-backfill, and neither has seen the match end — that
        is precisely the shape that produced a fabricated 273-203 result."""
        ...
        with self.conn:
            cur = self.conn.cursor()
            self._finalize_match(
                cur, mid, self._last_snap, reason,
                end_state="finalized" if confirmed else "unverified")
            self._flush_vehicle_sessions(cur)
    ...
    def _open_match(self, cur: sqlite3.Cursor, gs: dict, now: int) -> None:
        ...
        cur.execute(
            "UPDATE matches SET status='final', winner_team=NULL, "
            "end_state='unverified', end_reason='superseded by a newer match', "
            "ended_at=COALESCE(ended_at, started_at), "
            "duration_sec=MAX(0, COALESCE(ended_at, started_at) - started_at) "
    ...
    def _update_match_running(self, cur: sqlite3.Cursor, match_id: str,
        ...
        cur.execute(
            ...
            (now, len(players),
             ...
             (gs.get("layer") or {}).get("name") or gs.get("mapName"),
             gs.get("gameModeName") or gs.get("gameModeId"),
             _tickets((t1 or {}).get("tickets")), _tickets((t2 or {}).get("tickets")),
    ...
    def _finalize_match(self, cur: sqlite3.Cursor, match_id: str,
                        last_snap: Optional[dict], reason: str, *,
                        end_state: str = "unverified",
                        end_snap: Optional[dict] = None) -> None:
        """Close out a match.

        `end_state` is the whole point: "finalized" means a confirmed terminal
        state was actually seen, "unverified" means the match simply stopped
        being observed. Only the first may publish a winner — deriving one from
        a mid-match snapshot is how the archive ended up with 123 fabricated
        results, and those fed ELO too.
        """
        confirmed = end_state == "finalized"
        # Prefer the frame that carried the terminal state; fall back to the
        # last active frame and say so, so the provenance is never guessed at.
        src, src_note = (end_snap, "tickets=terminal-frame")
        t1, t2 = self._teams(src or {})
        a, b = _tickets((t1 or {}).get("tickets")), _tickets((t2 or {}).get("tickets"))
        if a is None or b is None:
            src, src_note = (last_snap, "tickets=last-active-frame")
            t1, t2 = self._teams(src or {})
            a = _tickets((t1 or {}).get("tickets"))
            b = _tickets((t2 or {}).get("tickets"))

        winner = None
        if confirmed and a is not None and b is not None:
            winner = 1 if a > b else (2 if b > a else 0)
        note = reason if not confirmed else f"{reason}; {src_note}"
        ...
        cur.execute(
            ...
            "  team2_faction=COALESCE(?, team2_faction) "
            "WHERE match_id=?",
            (now, now, winner, end_state, note[:200], a, b,
             (t1 or {}).get("factionId"), (t2 or {}).get("factionId"),
             match_id))
...
def list_matches(db_path: Path, limit: int = 50, period: str = "alltime",
    ...
    cutoff = period_cutoff(period, now)
    # A match whose end was never observed carries a mid-match snapshot as its
    # score. It stays in the DB and in every player's totals; it just does not
    # belong in a list of finished matches.
    where = ("m.started_at IS NOT NULL AND m.status='final' "
             "AND COALESCE(m.end_state,'legacy') <> 'unverified'")
    params: list[Any] = []
    if cutoff is not None:
        where += " AND m.started_at >= ?"
        params.append(cutoff)
