...
try:
    import numpy as _np
except ImportError:  # degraded mode: read_object_records_np() returns None
...
# Same 24-byte record as a numpy structured dtype — lets
# read_object_records_np() lift a whole chunk's (Object, Serial) columns
# out of the block bytes with zero per-slot Python work.
if _np is not None:
    _ITEM_DTYPE = _np.dtype([("pad0", "V8"), ("obj", "<u8"),
                             ("serial", "<i4"), ("pad1", "V4")])
    assert _ITEM_DTYPE.itemsize == UOBJECT_ITEM_SIZE
...
class GUObjectArray:
    ...
    def read_object_records_np(self):
        """
        Vectorized equivalent of iter_object_records(): one bulk read per
        chunk, then numpy lifts the (Object, Serial) columns straight out
        of the block bytes. Returns (addrs uint64[n], serials int32[n])
        indexed by InternalIndex, or None when numpy is unavailable or
        the topology is unreadable (callers fall back to the iterator).

        Slot semantics match the iterator exactly: a chunk that can't be
        read — or a bad chunk pointer — leaves its slice zeroed, which is
        indistinguishable from empty slots, the same "skip these objects
        this tick" behavior the iterator's `continue` produces
        (_read_item_block already zero-fills partial failures).
        """
        if _np is None:
            return None
        topo = self.refresh_topology()
        if topo is None:
            return None
        n = topo.num_elements
        if n <= 0:
            return None
        addrs = _np.zeros(n, dtype=_np.uint64)
        serials = _np.zeros(n, dtype=_np.int32)
        chunk_ptrs = topo.chunk_ptrs
        i = 0
        while i < n:
            ch = i // NUM_ELEMENTS_PER_CHUNK
            within = i % NUM_ELEMENTS_PER_CHUNK
            count = min(NUM_ELEMENTS_PER_CHUNK - within, n - i)
            chunk_ptr = chunk_ptrs[ch] if ch < len(chunk_ptrs) else 0
            if 0 < chunk_ptr <= 0x0000_7fff_ffff_ffff:
                block = _read_item_block(
                    self.pm, chunk_ptr + within * UOBJECT_ITEM_SIZE, count)
                if block is not None and len(block) == count * UOBJECT_ITEM_SIZE:
                    rec = _np.frombuffer(block, dtype=_ITEM_DTYPE, count=count)
                    addrs[i:i + count] = rec["obj"]
                    serials[i:i + count] = rec["serial"]
            i += count
        return addrs, serials
