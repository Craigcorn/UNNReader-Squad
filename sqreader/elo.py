...
def apply_match_elo(cur: sqlite3.Cursor, match_id: str, *,
    ...
    """
    m = cur.execute(
        "SELECT duration_sec, winner_team, COALESCE(end_state,'legacy') end_state, "
        "  (SELECT COUNT(*) FROM player_matches WHERE match_id = ?) AS n "
        "FROM matches WHERE match_id = ?", (match_id, match_id)).fetchone()
    if m is None:
        return 0
    duration, winner_team, end_state, n_players = m[0], m[1], m[2], m[3]
    if (n_players or 0) < min_players or (duration or 0) < min_duration:
        return 0
    # A match whose end was never observed has a MID-MATCH snapshot as its
    # result. Rating it bakes a fabricated outcome into a permanent running
    # total that nothing can reverse — the reason ratings had to be rebuilt.
    # Left unrated (elo_change stays NULL) so a later repair can pick it up.
    if end_state == "unverified":
        return 0
...
def recalc_all_elo(db_path: Path, *,
    ...
    try:
        ...
        matches = conn.execute(
            ...
            # Same rule as the live path: an unobserved ending contributes
            # nothing rather than contributing a fabricated one.
            "WHERE COALESCE(m.end_state,'legacy') <> 'unverified' "
            "  AND COALESCE(m.duration_sec,0) >= ? "
