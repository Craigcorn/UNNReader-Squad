...
def _load_process_vm_readv():
    ...
    if sys.platform != "linux":
        return None
    if os.environ.get("SQREADER_NO_PROCESS_VM_READV"):
        # Opt-out: keep the strict os.pread-only read path (read_many falls
        # back to a per-region pread loop, byte-for-byte identical). Set on
        # deployments that want the auditable "all reads go through the
        # O_RDONLY /proc/<pid>/mem fd, nothing else" guarantee.
        return None
    try:
